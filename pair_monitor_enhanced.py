...
import requests
from terminal_display import console, create_pair_table, create_security_table, log_message, open_new_terminal
            ...
            async with aiohttp.ClientSession(timeout=timeout) as session:
                async with session.get(url, params=params) as response:
                    if response.status == 200:
                        ...
                        log_message(f"Raw Honeypot API response: {data}", "INFO")
                        
                        if not isinstance(data, dict):
                            if attempt == max_retries - 1:
                                return {'error': 'Response is not a dictionary'}
                            log_message("Invalid response format (not a dict), retrying...", "WARNING")
                            continue
                        
                        # If simulationSuccess is False, create a honeypotResult
                        if not data.get('simulationSuccess', True):
                            data['honeypotResult'] = {
                                'isHoneypot': True,
                                'honeypotReason': data.get('simulationError', 'Unknown error')
                            }
                        # If no honeypotResult but simulation was successful
                        elif 'honeypotResult' not in data and data.get('simulationSuccess', False):
                            data['honeypotResult'] = {
                                'isHoneypot': False
                            }
                        ...
                        if 'honeypotResult' not in data:
                            ...
                            log_message("Invalid response format (no honeypotResult), retrying...", "WARNING")
                            continue
                    ...
                    if response.status == 429:  # Rate limit
                        ...
                        log_message(f"Rate limited, waiting {wait_time} seconds...", "WARNING")
...
def save_pair_to_db(event, token0_info, token1_info, pair_info):
    ...
    try:
        ...
        # Update session stats within same transaction
...
def save_security_check(token_address: str, honeypot_data: dict, goplus_data: dict):
    ...
    try:
        ...
        values = (
            ...
            safe_bool(honeypot_data.get('simulationSuccess')),
            str(honeypot_data.get('simulationError', '')),
            hp_raw_data,
...
def handle_event(event):
    ...
    table = create_pair_table(event, token0_info, token1_info, pair_info)
    console.print("\n")
    console.print(table)
    console.print("\n")
...
async def handle_event_with_security(event):
    ...
    try:
        ...
        try:
            ...
            log_message(f"Failed to broadcast to frontend: {str(e)}", "ERROR")
        
    except Exception as e:
        log_message(f"Error during security check: {str(e)}", "ERROR")

async def main_async():
    """Main async function with graceful shutdown"""
    # Open a new terminal window for output
    open_new_terminal()
    
    console.print("\n")
    console.rule("[bold magenta]🚀 UNISWAP V2 PAIR MONITOR[/]")
    console.print("\n")
    ...
    try:
        ...
        console.print("\n")
        console.rule("[bold magenta]MONITORING FOR NEW PAIRS[/]")
        console.print("\n")
        ...
        console.print("\n")
        console.rule("[bold red]SHUTTING DOWN[/]")
        console.print("\n")
        return 0
    except Exception as e:
...
def get_token_info(token_address):
    ...
    try:
        ...
        return {
            ...
            'verified': True  # Set to True by default since we're not checking
        }
